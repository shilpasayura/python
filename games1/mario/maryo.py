...
class dragon:
    ...
    def lost(self):
        self.imagerect.top=window_height+200
...
class maryo:
    ...
    def __init__(self):
        ...
        self.score = 0
        self.mx=self.imagerect.topleft[0]
        self.my = self.imagerect.topleft[1]
    ...
    def update(self):
        ...
        if (moveleft and (self.imagerect.left > self.speed+30)):
            self.imagerect.left -= self.speed

        if (moveright and (self.imagerect.left < window_width - 50)):
            self.imagerect.left += self.speed

        if (self.imagerect.left > window_width - 100):
            #mario wins
            self.score += 50
            self.imagerect.left=30
            pygame.display.set_caption('MARIO WINS')
            Dragon.lost()
